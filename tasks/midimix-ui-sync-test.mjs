import assert from "node:assert/strict";

class FakeClassList {
  constructor() {
    this.classNames = new Set();
  }

  add(...classNames) {
    classNames.forEach((className) => this.classNames.add(className));
  }

  remove(...classNames) {
    classNames.forEach((className) => this.classNames.delete(className));
  }

  contains(className) {
    return this.classNames.has(className);
  }

  toggle(className, force) {
    if (force === undefined) {
      if (this.classNames.has(className)) {
        this.classNames.delete(className);
        return false;
      }
      this.classNames.add(className);
      return true;
    }

    if (force) {
      this.classNames.add(className);
      return true;
    }

    this.classNames.delete(className);
    return false;
  }
}

class FakeElement {
  constructor({ id = null, classNames = [], dataset = {}, tagName = "DIV", type = "" } = {}) {
    this.id = id;
    this.dataset = { ...dataset };
    this.tagName = tagName;
    this.type = type;
    this.classList = new FakeClassList();
    this.attributes = new Map();
    this.children = [];
    this.textContent = "";
    this.disabled = false;
    this.value = "";
    this.checked = false;
    this.controllerRef = null;
    this.className = "";
    classNames.forEach((className) => this.classList.add(className));
  }

  setAttribute(name, value) {
    this.attributes.set(name, String(value));
  }

  getAttribute(name) {
    return this.attributes.get(name) ?? null;
  }

  replaceChildren(...children) {
    this.children = children;
  }

  append(...children) {
    this.children.push(...children);
  }

  addEventListener() {}

  removeEventListener() {}

  get options() {
    return this.children;
  }
}

class FakeDocument {
  constructor() {
    this.elementsById = new Map();
    this.selectorMap = new Map();
    this.activeElement = null;
  }

  registerElement(element, selectors = []) {
    if (element.id) {
      this.elementsById.set(element.id, element);
    }

    selectors.forEach((selector) => {
      if (!this.selectorMap.has(selector)) {
        this.selectorMap.set(selector, []);
      }
      this.selectorMap.get(selector).push(element);
    });

    return element;
  }

  getElementById(id) {
    return this.elementsById.get(id) ?? null;
  }

  createElement(tagName = "div") {
    return new FakeElement({ tagName: String(tagName).toUpperCase() });
  }

  querySelectorAll(selector) {
    return this.selectorMap.get(selector)?.slice() || [];
  }

  addEventListener() {}
}

const fakeDocument = new FakeDocument();
fakeDocument.registerElement(new FakeElement({ id: "status" }));
fakeDocument.registerElement(new FakeElement({ id: "global-play", tagName: "BUTTON", type: "button" }));
fakeDocument.registerElement(new FakeElement({ id: "global-stop", tagName: "BUTTON", type: "button" }));
fakeDocument.registerElement(new FakeElement({ id: "midi-status" }));
fakeDocument.registerElement(new FakeElement({ id: "midi-clock-status" }));
fakeDocument.registerElement(new FakeElement({ id: "midi-output-follower-status" }));
const midiInputPort = fakeDocument.registerElement(new FakeElement({ id: "midi-input-port", tagName: "SELECT" }));
const midiOutputPort = fakeDocument.registerElement(new FakeElement({ id: "midi-output-port", tagName: "SELECT" }));
const midiClockMode = fakeDocument.registerElement(new FakeElement({ id: "midi-clock-mode", tagName: "SELECT" }));
const midiInputControllerProfile = fakeDocument.registerElement(new FakeElement({ id: "midi-input-controller-profile" }));
const midimixAssignments = fakeDocument.registerElement(new FakeElement({ id: "midimix-controller-assignments" }));

midiClockMode.append(
  Object.assign(new FakeElement({ tagName: "OPTION" }), { value: "off", textContent: "Off" }),
  Object.assign(new FakeElement({ tagName: "OPTION" }), { value: "slave", textContent: "External" }),
  Object.assign(new FakeElement({ tagName: "OPTION" }), { value: "master", textContent: "Send" }),
);

const previousDocument = global.document;
const previousWindow = global.window;
const previousCustomEvent = global.CustomEvent;

global.document = fakeDocument;
global.window = { document: fakeDocument };
global.CustomEvent = class CustomEvent extends Event {
  constructor(type, params = {}) {
    super(type);
    this.detail = params.detail;
  }
};

try {
  const [
    constantsModule,
    { AudioStateController },
    { state },
    { bindControllerEvents },
  ] = await Promise.all([
    import("../js/constants.js"),
    import("../js/audio-state-controller.js"),
    import("../js/state.js"),
    import("../js/ui.js"),
  ]);

  const {
    DEFAULT_MIDI_CONTROLLER_ASSIGNMENTS_BY_CONTROL_ID,
    MIDIMIX_PROFILE_ID,
  } = constantsModule;

  state.midi.supported = true;
  state.midi.accessGranted = true;
  state.midi.availableInputs = [{ id: "input-1", name: "MIDI Mix", manufacturer: "AKAI Professional" }];
  state.midi.availableOutputs = [{ id: "output-1", name: "Output 1", manufacturer: "Test" }];
  state.midi.inputPortId = "input-1";
  state.midi.inputControllerProfileId = MIDIMIX_PROFILE_ID;
  state.midi.inputControllerProfileLabel = "AKAI MIDImix";
  state.midi.outputPortId = "output-1";
  state.midi.clockMode = "off";
  state.midi.remoteNoteOutputActive = false;
  state.transportState = "stopped";
  state.playingPresetIds.clear();
  state.midi.controllerAssignmentsByControlId = {
    ...DEFAULT_MIDI_CONTROLLER_ASSIGNMENTS_BY_CONTROL_ID,
    "midimix-knob-a-1": "filter-cutoff",
    "midimix-knob-a-2": "tempo-bpm",
  };

  const controller = new AudioStateController();
  bindControllerEvents(controller);

  controller.emitStateChange("midi-settings-updated", {});

  assert.equal(midiInputPort.value, "input-1", "the MIDI input select should stay synced");
  assert.equal(midiOutputPort.value, "output-1", "the MIDI output select should stay synced");
  assert.equal(midiInputControllerProfile.textContent, "AKAI MIDImix active", "the MIDI panel should show the detected MIDImix profile");
  assert.equal(midiInputControllerProfile.classList.contains("is-active"), true, "the MIDImix profile label should use the active highlight");
  assert.equal(midimixAssignments.children.length, 3, "the MIDImix assignment panel should render one section per knob row");

  const firstSection = midimixAssignments.children[0];
  const firstRowGrid = firstSection.children[1];
  assert.equal(firstRowGrid.children.length, 8, "each MIDImix row should render 8 assignable knob slots");
  assert.equal(firstRowGrid.children[0].children[1].value, "filter-cutoff", "the first knob assignment should sync its selected control");
  assert.equal(firstRowGrid.children[1].children[1].value, "tempo-bpm", "the second knob assignment should sync its selected control");

  console.log("midimix UI sync checks passed");
} finally {
  global.document = previousDocument;
  global.window = previousWindow;
  global.CustomEvent = previousCustomEvent;
}

